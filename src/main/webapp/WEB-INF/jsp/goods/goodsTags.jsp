...
<body>
<div class="pd-20">
    <div class="form form-horizontal" id="form-goodsTags-add">
        <div class="cl pl-20 pt-10 pb-10">
            <div class="text-r">
                <input type="text" id="goods_tags_add"  style="width:200px" class="input-text" placeholder="请输入商品标签名称"/>
                <button onclick="add()" class="btn btn-primary radius">新增</button>
            </div>
        </div>
        <div class="pd-20 clearfix">
            <table class="table table-border table-bordered table-bg table-hover table-striped box-shadow" id="goods_tags_table">
                <thead>
                <tr class="text-c">
                    <th width="50">标签</th>
                    <th width="50">操作</th>
...
<script>
    var table;
    $(function(){
        table = $('#goods_tags_table').dataTable({
            ...
            "sAjaxSource" : "<%=request.getContextPath()%>/goods/goodsTagsList",
            //服务器端，数据回调处理
            ...
            "fnDrawCallback" : function () {
                ...
                $(".editDiv").editable("<%=request.getContextPath()%>/goods/editGoodsTags", {
                    width   : 120,
                    height  : 18,
                    cancel : "取消",
                    submit : "保存",
...
//添加
function add() {
    var tagsName = $.trim($("#goods_tags_add").val());
    $.post("<%=request.getContextPath()%>/goods/addGoodsTags", {"name":tagsName}, function (data) {
        if(data.result == 1){
            layer.msg("标签添加成功");
            $("#goods_tags_add").val("");
            table.fnDraw();
