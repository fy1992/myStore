...
<div class="pd-20 minwidth">
    <form class="form form-horizontal" id="form-netStore-edit" action="<%=request.getContextPath()%>/server/store/edit" method="POST">
        <div class="row cl">
            <label class="form-label col-3">网店状态：</label>
            <div class="formControls col-6">
                <div class="radio-box">
                    <input type="radio" id="store_using-1" name="status" value = "1" <c:if test="${store.status eq 1}">checked</c:if>>
                    <label for="store_using-1">开启</label>
                </div>
                <div class="radio-box">
                    <input type="radio" id="store_using-2" name="status" value = "0" <c:if test="${store.status eq 0}">checked</c:if>>
                    <label for="store_using-2">关闭</label>
                </div>
            </div>
            <div class="col-3"></div>
        </div>
        <div class="row cl">
            <label class="form-label col-3">网店URL：</label>
            <div class="formControls col-6">
                <input type="text" name = "netStoreUrl" class="input-text radius">
            </div>
            <div class="col-3"> </div>
        </div>
        <div class="row cl">
        ...
        <div class="row cl">
            <div class="col-10 col-offset-5 mt-20">
                <input class="btn btn-primary radius" type="submit" id="netEditBtn" value="&nbsp;&nbsp;&nbsp;&nbsp;确认&nbsp;&nbsp;&nbsp;&nbsp;">
            </div>
        </div>
    </form>
</div>
...
<script>
    $(function () {
        var  validtor = $("#form-netStore-edit").Validform({
            tiptype:4,
            showAllError:true,
            ajaxPost: true,
