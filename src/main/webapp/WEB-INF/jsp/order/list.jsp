...
<body class="pos-r">
<div>
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 网店 <span class="c-gray en">&gt;</span> 网店订单 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div class="clearfix">
        <div class="text-r cl pl-20 pt-10 pb-10 box-shadow">
            <span class="select-box" style="width: 100px;">
                <select class="select" id="status">
                    ...
                    <option value = "0">待送货</option>
                    <option value = "1">已送货</option>
                    <option value = "2">已完成</option>
                </select>
            </span>
            <span class="select-box" style="width: 100px;">
                <select class="select" id="payStatus">
            ...
            <input type="text" id="orderNo" placeholder="订单号" style="width:160px" class="input-text radius">
            <button id="order_search" class="btn btn-success"><i class="Hui-iconfont">&#xe665;</i> 查询</button>
        </div>
        <div class="pd-20 clearfix">
            <table class="table table-border table-bordered table-bg table-hover table-striped box-shadow" id="order_table">
                <thead>
                    <tr class="text-c">
                        <th width="50">操作</th>
                        <th width="100">订单号</th>
...
table = $('#order_table').dataTable({
       ...
       "aoColumns" : [
        ...
        {"mData" : "clientOrderNo", "sDefaultContent" : "", "bSortable":false},
        {"mData" : "orderTime", "sDefaultContent" : "", "bSortable":false, "mRender":function (data, type, full) {
            return format(data);
        }},
        {"mData" : "fromType", "sDefaultContent" : "", "bSortable":false, "mRender":function(data, type, full){
            return data == 0 ? "自营" : "-";
        }},
        {"mData" : "status", "sDefaultContent" : "", "bSortable":false, "mRender":function(data, type, full){
            return data == 0 ? "待发货" : data == 1 ? "已发货" : data == 2 ? "已完成" : "已作废";
        }},
        {"mData" : "orderName", "sDefaultContent" : "", "bSortable":false},
        {"mData" : "phone", "sDefaultContent" : "", "bSortable":false},
        {"mData" : "orderAddr", "sDefaultContent" : "", "bSortable":false},
        {"mData" : "orderNums", "sDefaultContent" : "", "bSortable":false},
    ...
    "language":{
       ...
       "sAjaxSource" : "<%=request.getContextPath()%>/server/order/list",
       //服务器端，数据回调处理  
    ...
    "fnServerParams" : function(aoData){  //那个函数是判断字符串中是否含有数字
        ...
        //var storeId = $("#orderStore").val();
        aoData.push({"name":"status","value":status});
        aoData.push({"name":"payStatus","value":payStatus});
...
//编辑
function edit(id){
    layer_show("编辑用户", "<%=request.getContextPath()%>/server/order/edit/"+id, "700", "600");
}
</script>
</body>
</html>